// Profile Manager for Quotely-Hardy - UPDATED for secure backend
class ProfileManager {
    ...
    async loadProfile() {
        try {
            const result = await QuotelyAPI.getUserProfile();
    ...
    displayProfile() {
        ...
        if (avatarElement && this.userProfile?.avatar_url) {
            ...
            const name = this.userProfile?.full_name || user.name || user.email.split('@')[0];
    ...
    async loadUserStats() {
        try {
            const result = await QuotelyAPI.getDashboardStats();
    ...
    async loadUserSubmissions() {
        // This would load user's submitted quotes from backend
        // For now, we'll show the empty state
        this.showEmptySubmissions();
    }

    async loadUserActivity() {
        // This would load user activity from backend
    ...
    openEditProfileModal() {
        ...
        if (modal && form) {
            ...
            form.querySelector('#editName').value = this.userProfile?.full_name || 
                                                    QuotelyAPI.getCurrentUser()?.name || 
                                                    '';
    ...
    async handleEditProfile(form) {
        ...
        try {
            const result = await QuotelyAPI.updateUserProfile(profileData);
    ...
    async handleSettingsForm(form) {
        ...
        try {
            const result = await QuotelyAPI.updateUserProfile(settingsData);
    ...
    async uploadAvatar(file) {
        // Note: This requires backend storage setup
